...
import { db, usersTable, teamsTable, tasksTable, submissionsTable, invitationsTable, notificationsTable, supervisorRequestsTable, meetingsTable, teamMembersTable, activityLogsTable } from "@workspace/db";
...
router.get("/dashboard/supervisor", requireAuth, async (req, res): Promise<void> => {
  ...
  if (teamIds.length > 0) {
    ...
    if (taskIds.length > 0) {
      ...
      pendingReviews = allSubs.filter(s => taskIds.includes(s.taskId) && s.status === "pending").length;
    }
  }
...
router.get("/dashboard/coordinator", requireAuth, async (req, res): Promise<void> => {
