...
router.get("/invitations", requireAuth, async (req, res): Promise<void> => {
  ...
  const formatted = await Promise.all(invitations.map(formatInvitation));
  res.json(formatted);
});
...
router.post("/invitations/:id/accept", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!inv || inv.invitedUserId !== req.user!.id) { res.status(404).json({ error: "Invitation not found" }); return; }
